...
public class UpdateByPrimaryKeyWithBLOBsMethodGenerator extends AbstractDAOElementGenerator {
	public void addImplementationElements(TopLevelClass topLevelClass) {
		...
		sb.append(this.daoTemplate.getUpdateMethod(table.getSqlMapNamespace(),
				XmlConstants.UPDATE_BY_PRIMARY_KEY_WITH_BLOBS_STATEMENT_ID, "record"));
		method.addBodyLine(sb.toString());

		method.addBodyLine("return rows;");

		if (this.ibatorContext.getPlugins().daoUpdateByPrimaryKeyWithBLOBsMethodGenerated(method, topLevelClass,
	...
	public void addInterfaceElements(Interface interfaze) {
		...
		if (this.ibatorContext.getPlugins().daoUpdateByPrimaryKeyWithBLOBsMethodGenerated(method, interfaze,
				this.introspectedTable)) {
	...
	private Method getMethodShell(Set<FullyQualifiedJavaType> importedTypes) {
		...
		method.setName(getDAOMethodNameCalculator().getUpdateByPrimaryKeyWithBLOBsMethodName(this.introspectedTable));
