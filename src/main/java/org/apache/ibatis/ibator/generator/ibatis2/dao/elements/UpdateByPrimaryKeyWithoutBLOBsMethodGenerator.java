...
public class UpdateByPrimaryKeyWithoutBLOBsMethodGenerator extends AbstractDAOElementGenerator {
	public void addImplementationElements(TopLevelClass topLevelClass) {
		...
		sb.append(this.daoTemplate.getUpdateMethod(table.getSqlMapNamespace(),
				XmlConstants.UPDATE_BY_PRIMARY_KEY_STATEMENT_ID, "record"));
		method.addBodyLine(sb.toString());

		method.addBodyLine("return rows;");

		if (this.ibatorContext.getPlugins().daoUpdateByPrimaryKeyWithoutBLOBsMethodGenerated(method, topLevelClass,
	...
	public void addInterfaceElements(Interface interfaze) {
		...
		if (this.ibatorContext.getPlugins().daoUpdateByPrimaryKeyWithoutBLOBsMethodGenerated(method, interfaze,
				this.introspectedTable)) {
	...
	private Method getMethodShell(Set<FullyQualifiedJavaType> importedTypes) {
		FullyQualifiedTable table = this.introspectedTable.getFullyQualifiedTable();
		FullyQualifiedJavaType parameterType = this.introspectedTable.getBaseRecordType();
		...
		method.setName(
				getDAOMethodNameCalculator().getUpdateByPrimaryKeyWithoutBLOBsMethodName(this.introspectedTable));
